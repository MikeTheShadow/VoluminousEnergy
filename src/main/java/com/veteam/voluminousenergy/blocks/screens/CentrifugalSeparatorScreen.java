...
public class CentrifugalSeparatorScreen extends VEContainerScreen<CentrifugalSeparatorContainer> {
    ...
    protected void renderSlotAndTankLabels(PoseStack matrixStack, int mouseX, int mouseY) {
        ...
        this.font.drawShadow(matrixStack, (TextUtil.translateString("gui.voluminousenergy.slot_short").copy().append("2")), 99F, 33F, WHITE_TEXT_COLOUR);
        this.font.drawShadow(matrixStack, (TextUtil.translateString("gui.voluminousenergy.slot_short").copy().append("3")), 117F, 15F, WHITE_TEXT_COLOUR);
        this.font.drawShadow(matrixStack, (TextUtil.translateString("gui.voluminousenergy.slot_short").copy().append("4")), 135F, 33F, WHITE_TEXT_COLOUR);
        this.font.drawShadow(matrixStack, (TextUtil.translateString("gui.voluminousenergy.slot_short").copy().append("5")), 117F, 51F, WHITE_TEXT_COLOUR);
    }
            ...
            tileEntity.getEnergy().ifPresent((veEnergyStorage -> {
                renderTooltip(matrixStack, Component.nullToEmpty(
                        veEnergyStorage.getEnergyStored()
                                + " FE / " + Config.CENTRIFUGAL_SEPARATOR_MAX_POWER.get()
                                + " FE"
                ), mouseX, mouseY);
            }));
        } else if (!VoluminousEnergy.JEI_LOADED && isHovering(78, 32, 14, 23, mouseX, mouseY)) {
