...
public class DistillationUnitScreen extends VEContainerScreen<VEContainer> {
    ...
    protected void renderBg(GuiGraphics matrixStack, float partialTicks, int mouseX, int mouseY) {
        ...
        if (tileEntity != null && processor.isMultiBlockValid(tileEntity)) {
            ...
            try {
                VERender.renderGuiTank(tileEntity.getLevel(), tileEntity.getBlockPos(), tileEntity.getFluidStackFromTank(1), tileEntity.getTankCapacity(1), i + 119, j + 11, 0, 12, 50);
            } catch (Exception e) {
            }

            try {
                VERender.renderGuiTank(tileEntity.getLevel(), tileEntity.getBlockPos(), tileEntity.getFluidStackFromTank(2), tileEntity.getTankCapacity(2), i + 157, j + 11, 0, 12, 50);
            } catch (Exception e) {
            }
            drawIOSideHelper();
            // Upgrade slot
