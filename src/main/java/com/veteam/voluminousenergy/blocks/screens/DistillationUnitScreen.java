...
public class DistillationUnitScreen extends VEContainerScreen<VEContainer> {
    ...
    private final Identifier GUI = Identifier.fromNamespaceAndPath(VoluminousEnergy.MODID, "textures/gui/distillation_unit_gui.png");
    ...
    protected void extractTooltip(GuiGraphicsExtractor matrixStack, int mouseX, int mouseY) {
        if (isHovering(11, 16, 12, 49, mouseX, mouseY)) {
            matrixStack.setTooltipForNextFrame(this.font, TextUtil.powerBarTooltip(tileEntity.getEnergy(), Config.DISTILLATION_UNIT_MAX_POWER.get()), mouseX, mouseY);
    ...
    public void extractBackground(GuiGraphicsExtractor matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        if (tileEntity != null && processor.isMultiBlockValid(tileEntity)) {
            ...
            try {
                VERender.renderGuiTank(matrixStack, tileEntity.getLevel(), tileEntity.getBlockPos(), tileEntity.getFluidStackFromTank(1), tileEntity.getTankCapacity(1), i + 119, j + 11, 0, 12, 50);
            } catch (Exception e) {
            }

            try {
                VERender.renderGuiTank(matrixStack, tileEntity.getLevel(), tileEntity.getBlockPos(), tileEntity.getFluidStackFromTank(2), tileEntity.getTankCapacity(2), i + 157, j + 11, 0, 12, 50);
            } catch (Exception e) {
            }
            drawIOSideHelper();
            // Upgrade slot
            ...
            matrixStack.blit(RenderPipelines.GUI_TEXTURED, MULTIBLOCK_WARN, i, j, 0, 0, 174, 82, 256, 256);
