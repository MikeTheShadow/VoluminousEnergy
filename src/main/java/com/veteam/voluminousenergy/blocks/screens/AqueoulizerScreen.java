...
public class AqueoulizerScreen extends VEContainerScreen<VEContainer> {
    ...
    private final Identifier GUI = Identifier.fromNamespaceAndPath(VoluminousEnergy.MODID, "textures/gui/aqueoulizer_gui.png");
    ...
    protected void extractTooltip(GuiGraphicsExtractor matrixStack, int mouseX, int mouseY) {
        if (isHovering(11, 16, 12, 49, mouseX, mouseY)) {
            matrixStack.setTooltipForNextFrame(this.font, TextUtil.powerBarTooltip(tileEntity.getEnergy(), Config.AQUEOULIZER_MAX_POWER.get()), mouseX, mouseY);
    ...
    public void extractBackground(GuiGraphicsExtractor matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        if (tileEntity != null) {
             ...
             */
            matrixStack.blit(RenderPipelines.GUI_TEXTURED, this.GUI, i + 127, j + 31, 176, 0, progress, 17, 256, 256);
            ...
            try {
                VERender.renderGuiTank(matrixStack, tileEntity.getLevel(), tileEntity.getBlockPos(), tileEntity.getFluidStackFromTank(1), tileEntity.getTankCapacity(1), i + 157, j + 18, 0, 12, 50);
            } catch (Exception e) {
            }
            drawIOSideHelper();
            // Upgrade slot
