...
public class ElectrolyzerScreen extends ContainerScreen<ElectrolyzerContainer> {
    ...
    protected void init(){
        ...
        // Bucket Insert
        this.addButton(new SlotBoolButton(tileEntity.bucketSm, (this.width/2)-198, this.guiTop+20, button ->{
            // Do nothing
        }));

        this.addButton(new SlotDirectionButton(tileEntity.bucketSm, (this.width/2)-184, this.guiTop+20, button ->{
            // Do nothing
        }));

        // Output
        this.addButton(new SlotBoolButton(tileEntity.outputSm, (this.width/2)-198, this.guiTop+40, button ->{
            // Do nothing
        }));

        this.addButton(new SlotDirectionButton(tileEntity.outputSm, (this.width/2)-184, this.guiTop+40, button ->{
            // Do nothing
        }));

        // RNG 1
        this.addButton(new SlotBoolButton(tileEntity.rngOneSm, (this.width/2)-198, this.guiTop+60, button ->{
            // Do nothing
        }));

        this.addButton(new SlotDirectionButton(tileEntity.rngOneSm, (this.width/2)-184, this.guiTop+60, button ->{
            // Do nothing
        }));

        // RNG 2
        this.addButton(new SlotBoolButton(tileEntity.rngTwoSm, (this.width/2)-198, this.guiTop+80, button ->{
            // Do nothing
        }));

        this.addButton(new SlotDirectionButton(tileEntity.rngTwoSm, (this.width/2)-184, this.guiTop+80, button ->{
            // Do nothing
        }));

        // RNG 3
        this.addButton(new SlotBoolButton(tileEntity.rngThreeSm, (this.width/2)-198, this.guiTop+100, button ->{
            // Do nothing
        }));

        this.addButton(new SlotDirectionButton(tileEntity.rngThreeSm, (this.width/2)-184, this.guiTop+100, button ->{
            // Do nothing
        }));
    }
    ...
    protected void drawGuiContainerForegroundLayer(MatrixStack matrixStack,int mouseX, int mouseY) {
        ...
        this.font.func_243246_a(matrixStack, TextUtil.translateVEBlock("electrolyzer"), 8.0F, 6.0F, 16777215);
    ...
    protected void renderHoveredTooltip(MatrixStack matrixStack,int mouseX, int mouseY) {
        if (isPointInRegion(11, 16, 12, 49, mouseX, mouseY)) {
            renderTooltip(matrixStack, ITextComponent.getTextComponentOrEmpty(container.getEnergy() + " FE" + " / " + Config.ELECTROLYZER_MAX_POWER.get() + " FE"), mouseX, mouseY);
        }
