...
public class ImplosionCompressorScreen extends VEContainerScreen<VEContainer> {
    ...
    private final Identifier GUI = Identifier.fromNamespaceAndPath(VoluminousEnergy.MODID, "textures/gui/implosion_compressor_gui.png");
    ...
    protected void extractLabels(@NotNull GuiGraphicsExtractor matrixStack, int mouseX, int mouseY) {
        TextUtil.renderShadowedText(matrixStack, this.font, TextUtil.translateVEBlock("implosion_compressor"), 8, 6, WHITE_TEXT_STYLE);
    ...
    protected void extractTooltip(GuiGraphicsExtractor matrixStack, int mouseX, int mouseY) {
        if (isHovering(11, 16, 12, 49, mouseX, mouseY)) {
            matrixStack.setTooltipForNextFrame(this.font, TextUtil.powerBarTooltip(tileEntity.getEnergy(), Config.IMPLOSION_COMPRESSOR_MAX_POWER.get()), mouseX, mouseY);
    ...
    public void extractBackground(GuiGraphicsExtractor matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        if (tileEntity != null) {
             ...
             */
            matrixStack.blit(RenderPipelines.GUI_TEXTURED, GUI, i + 78, j + 31, 176, 0, progress, 17, 256, 256);
