...
public class MultiFluidSlotWrapper implements IFluidHandler {
    ...
    public MultiFluidSlotWrapper(List<RelationalTank> tanks) {
        ...
        tanks.forEach(t -> tankHashMap.put(t.getId(), t));
    }
    ...
    public boolean isFluidValid(int tank, @Nonnull FluidStack stack) {
        ...
        if (relationalTank.getValidFluids().stream().noneMatch(fluid -> fluid.isSame(stack.getFluid())) && !relationalTank.isAllowAny())
            return false;
    ...
    public FluidStack drain(FluidStack resource, FluidAction action) {
        ...
        for (RelationalTank tank : tanks) {
            if (tank.getTankType() == TankType.INPUT && !Config.ALLOW_EXTRACTION_FROM_INPUT_TANKS.get() && !tank.isAllowAny())
                continue;
